...
export function Onboarding({
  ...
  const panGesture = Gesture.Pan()
    ...
    .onEnd((event) => {
      ...
      const shouldSwipe = Math.abs(translationX) > screenWidth * 0.3 || Math.abs(velocityX) > 400;
    ...
    return (
      ...
      >
        <View style={styles.contentContainer}>
          {step.image && <View style={styles.imageContainer}>{step.image}</View>}

          {step.icon && !step.image && <View style={styles.imageContainer}>{step.icon}</View>}
